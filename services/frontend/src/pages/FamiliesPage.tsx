...
function FamilyModal({ initial, onClose, onSaved }: { initial: FamilyRead | null; onClose: () => void; onSaved: () => void }) {
  ...
  async function handleSave() {
    ...
    setSaving(true)
    try {
      const body: FamilyWrite = { name: name.trim(), sort_order: Number(sortOrder) || 0 }
...
export default function FamiliesPage() {
  ...
  function reload() {
    ...
    Promise.all([
      ...
      productsApi.list({ only_active: false, limit: 1000 }),
    ])
        ...
        {loading ? <p className="text-sm text-gray-400">Carregando...</p> : filtered.length === 0 ? <p className="text-sm text-gray-400">Nenhuma família.</p> : (
          <table className="w-full text-sm">
            <thead><tr className="border-b border-gray-100 dark:border-gray-700">
              ...
              <th className="text-right pb-2 text-xs font-bold uppercase tracking-wider text-gray-600 dark:text-gray-300 w-28">Produtos</th>
              <th className="text-center pb-2 text-xs font-bold uppercase tracking-wider text-gray-600 dark:text-gray-300 w-24">Status</th>
              <th className="w-20" />
            </tr></thead>
            <tbody>
              {filtered.map((f, i) => (
                <tr key={f.id} className={`hover:bg-gray-100 dark:hover:bg-gray-700/50 ${i % 2 === 0 ? 'bg-gray-50 dark:bg-gray-700/30' : ''}`}>
                  <td className="py-3 pl-3 font-semibold text-gray-800 dark:text-gray-100">{f.name}</td>
                  <td className="py-3 text-right text-gray-500 dark:text-gray-400">{f.sort_order}</td>
                  ...
                  <td className="py-3 pr-3 text-right">
                    <button onClick={() => { setEditing(f); setOpen(true) }} className="text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 p-1" title="Editar"><Pencil size={16} /></button>
                    {f.active && <button onClick={() => handleSoftDelete(f)} className="text-gray-400 hover:text-red-600 p-1 ml-1" title="Desativar"><Trash size={16} /></button>}
                  </td>
                </tr>
              ))}
            </tbody>
